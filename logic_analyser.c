...
static uint32_t capture_size_words;

static inline uint bits_packed_per_word() {
...
static void logic_analyser_pio_setup(float div) {
    ...
    sm_config_set_in_shift(&c, true, true, bits_packed_per_word());
...
void logic_analyser_print_capture_buf(int min_sample) {
    ...
    // 01: ____----____----____----
    printf("Capture of %d samples %s:\n", n_samples, dma_channel_is_busy(dma_chan) ? "not done" : "done");
    ...
    for (int pin = 0; pin < pin_count; ++pin) {
        ...
        for (int sample = min_sample; sample < n_samples; ++sample) {
            ...
            printf(capture_buf[word_index] & word_mask ? "-" : "_");
        }
        printf("\n");
    }
}
...
void logic_analyser_init(PIO pio_, int pin_base_, int pin_count_, int n_samples_, float div) {
    pio = pio_;
    pin_base = pin_base_;
    pin_count = pin_count_;
    n_samples = n_samples_;

    // We're going to capture into a u32 buffer, for best DMA efficiency. Need
    // to be careful of rounding in case the number of pins being sampled
    // isn't a power of 2.
    uint total_sample_bits = n_samples * pin_count;
    total_sample_bits += bits_packed_per_word() - 1;
    capture_size_words = total_sample_bits / bits_packed_per_word();
    capture_buf = malloc(capture_size_words * sizeof(uint32_t));
